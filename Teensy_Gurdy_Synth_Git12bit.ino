...
#define MUXfilres  12
#define MUXmotor   13 //this is only used in an old version but I don't want to cut stuff out in case I break something
#define MUXvolume  14
#define MUXenv     15
...
void checkMux() { //This code is based on the Notes & Volts Teensy synth project. Go check out our old pal Dave!
  ...
  if (currentMicros >= 1000) {
    ...
    int muxRead = analogRead(MZ);
      ...
      switch (muxInput) {
        case MUXpitch1:
          muxRead = map (muxRead, 0, 4095, 20, 200); //This frequency range of ~E0-G3 bounds the bass strings
          waveform1.frequency(muxRead);
          break;
        case MUXpitch2:
          muxRead = map (muxRead, 0, 4095, 20, 200);
        ...
        case MUXpitch3:
          ...
          break;
        case MUXpitch4:
          muxRead = map (muxRead, 0, 4095, 150, 1400); //This frequency range of ~D3-F6 bounds the treble strings
          waveform4.frequency(muxRead);
          break;
        case MUXpitch5:
          muxRead = map (muxRead, 0, 4095, 150, 1400);
        ...
        case MUXpitch12:
          ...
          break;
        case MUXfilres:
          muxRead = map (muxRead, 0, 4095, 0.7, 5.0);
          filter1.resonance(muxRead); 
          break;
        case MUXvolume:
          amp = float(muxRead);
          amp = map (amp, 0.0, 4095.0, 0.0, 1.0);
          amp1.gain(amp);
          break;
        case MUXenv:
          muxRead = map (muxRead, 0, 4095, 50, 1000);
